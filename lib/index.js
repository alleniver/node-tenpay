...
class Payment {
  constructor(
    ...
    this.urls = sandbox
      ? {
          ...
          sendminiprogramhb: 
            "https://api.mch.weixin.qq.com/sandboxnew/mmpaymkttransfers/sendminiprogramhb",
          sendgroupredpack:
  ...
  middleware(type = "pay") {
    return async (ctx, next) => {
      ...
      try {
        const xmlData = ctx.request.xmlBody.xml;
        Object.keys(xmlData).forEach(key => {
          xmlData[key] = xmlData[key][0];
        });

        ctx.request.weixin = await this._parse(xmlData, "middleware_" + type);
      } catch (err) {
      ...
      let pkg = {
        ...
        mch_billno:
          ...
          (params.mch_autono
            ...
            : ""),
        total_num: params.total_num || 1,
        scene_id:params.scene_id || 'PRODUCT_1',
        notify_way:params.notify_way || 'MINI_PROGRAM_JSAPI'
      };
      delete pkg.mch_autono;
  
      return this._request(pkg, "sendminiprogramhb", true);
    }
