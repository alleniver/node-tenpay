...
class Payment {
  ...
  async _parse(data, type, signType, isJson = false) {
    let json;
    if (isJson) {
      json = data;
    }else {
      json  = await util.parseXML(data);
    }
  ...
  middleware(type = 'pay') {
    return async (ctx, next) => {
      ...
      try {
        let xmlData = ctx.request.xmlBody.xml;
        Object.keys(xmlData).forEach(key => {
          xmlData[key] = xmlData[key][0];
        });
        ctx.request.weixin = await this._parse(xmlData, 'middleware_' + type, true);
      } catch (err) {
